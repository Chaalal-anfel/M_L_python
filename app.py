...
def streamlit_menu(example=1):
    if example == 2:
        ...
        selected = option_menu(
            ...
            options=["Driving behaviour","Driving behaviour many traject","Graphs","Driver Analyses"],  # required
            icons=["bi-truck","bi-truck", "bi-graph-up","bi-pie-chart-fill"],  # optional
...
if selected == "Driving behaviour many traject":
    st.title(f" {selected}:")
    with open('style.css') as f:
     st.markdown(f'<style>{f.read()}</style>', unsafe_allow_html=True)
     st.write('''
     # Classification trajets conducteurs pour plusieurs jours:
     ''')   
    ...
    if  not df.empty:
        ...
        st.text('vous devez entrer les trajets à prédire !!!! ')
...
if selected == "Driving behaviour":
    ...
    if (df1 == 0).sum(axis=1).any():
        st.text('vous devez remplir tous les champs')
    else:
        liste=['agressif driving(reduce your acceleration and increase your speed)',
        'agressif driving(reduce your acceleration)',
        'bad driving',
